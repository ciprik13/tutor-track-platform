...
function guessStudentId(
  eventTitle: string,
  students: { id?: any; name: string }[],
): string | null {
  ...
  for (const student of students) {
    ...
    if (
      tokens.some((token) =>
        parts.some((part) => part.length > 1 && part === token),
      )
    ) {
...
function mergeConsecutiveEvents(
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (let j = i + 1; j < sorted.length; j++) {
      const nxt = sorted[j];
      if (consumed.has(nxt.id) || !nxt.start.dateTime || !nxt.end.dateTime)
        continue;
      ...
      const totalMin =
        (new Date(nxt.end.dateTime).getTime() -
          new Date(merged.start.dateTime!).getTime()) /
        60000;
      ...
      merged = {
        ...
        mergedIds: [...merged.mergedIds!, nxt.id],
      };
...
function detectDuplicate(
  ...
  if (
    allIds.some((id) =>
      existingLessons.some((l: any) => l.googleCalendarEventId === id),
    )
  ) {
    return "exact";
  }
  if (studentId && event.start.dateTime) {
    ...
    if (
      existingLessons.some(
        (l: any) =>
          l.studentId === studentId && l.date.slice(0, 13) === eventHour,
      )
    ) {
      return "probable";
    }
...
export default function CalendarImport({ onClose }: Props) {
      ...
      new Set(
        merged
          .filter((e) => {
            ...
            return (
              sid !== null && detectDuplicate(e, sid, existingLessons) === null
            );
          })
          .map((e) => e.id),
      ),
    ...
    try {
      ...
      if (err.message === "TOKEN_EXPIRED") {
        try {
          ...
          setError(
            "Tokenul a expirat și reconectarea a eșuat. Reconectează din Setări.",
          );
        }
      } else {
  ...
  const handleImport = async () => {
    ...
    for (const event of toImport) {
      ...
      const price =
        duration === 60
          ? profile.defaultPrice60
          : duration === 90
            ? profile.defaultPrice90
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
                ...
                >
                  ...
                  {duplicateCount > 0 && (
                    ...
                    >
                      {duplicateCount}{" "}
                      {duplicateCount === 1
                        ? "eveniment există deja"
                        : "evenimente există deja"}{" "}
                      în platformă — neselectate automat. Poți să le bifezi
                      manual dacă vrei să le reimportezi.
                    </div>
                  )}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            ...
                            {event.mergedIds && event.mergedIds.length > 1 && (
                              ...
                              >
                                {event.mergedIds.length}×
                                {Math.round(duration / event.mergedIds.length)}{" "}
                                min combinat
                              </span>
                            )}
                            {event.calendarName && (
                          ...
                          {isDup ? (
                            <span
                              style={{
                                ...
                                borderRadius: 20,
                                border:
                                  "0.5px solid color-mix(in srgb, var(--warning) 30%, transparent)",
                              }}
                            >
                              {dupKind === "exact"
                                ? "Deja importat"
                            ...
                            <span
                              className="tt-pill tt-pill-active"
                              style={{ fontSize: 10.5 }}
                            >
          ...
          {step === "done" && (
            <div style={{ textAlign: "center", padding: "32px 0" }}>
              ...
              <div
                style={{ fontSize: 13, color: "var(--text-3)", marginTop: 6 }}
              >
                {importedCount}{" "}
                {importedCount === 1 ? "lecție adăugată" : "lecții adăugate"}
              </div>
            </div>
          )}
        </div>
