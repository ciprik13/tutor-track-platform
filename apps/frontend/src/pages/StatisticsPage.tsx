...
export default function StatisticsPage() {
  ...
  const yearLessons = (allLessons as any[]).filter((l: any) =>
    l.date.startsWith(String(currentYear)),
  );
      ...
      >
        ...
        {payload.map((p: any) => (
          ...
          >
            ...
            {p.name !== "lectii" ? ` ${profile.currency}` : ""}
          </p>
        ))}
      </div>
    );
              ...
              {durationData.map((d) => {
                const pct =
                  yearLessons.length > 0
                    ? Math.round((d.value / yearLessons.length) * 100)
                    : 0;
                return (
                  <div
                    key={d.name}
