...
export default function LessonsPage() {
  ...
  const { data: googleStatus } = useQuery({ queryKey: ["google-status"], queryFn: googleApi.getStatus, retry: false });
    ...
    <div style={{ padding: isMobile ? "20px 16px 60px" : "28px 36px 60px", maxWidth: 1280 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          {googleConnected && (
            <button onClick={() => setCalendarImport(true)} className="tt-btn tt-btn-secondary" style={{ height: 38, gap: 7 }}>
              <IcCalendar /> {isMobile ? "Import" : "Import Calendar"}
            </button>
          )}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap", alignItems: "center" }}>
        <select
          ...
          className="tt-input"
          style={{ width: isMobile ? "100%" : "auto", minWidth: isMobile ? "unset" : 200 }}
        >
        ...
        <div className="tt-filter-row">
          {([
            ...
            <button key={k ?? "all"} onClick={() => setPaymentFilter(k)} className={`tt-filter-btn ${paymentFilter === k ? "active" : ""}`}>
              {lbl}
            </button>
          ))}
        </div>
            ...
            return (
              <div key={day} className="tt-card" style={{ padding: 0, overflow: "hidden", marginBottom: 10 }}>
                ...
                <div style={{
                  ...
                  background: "var(--bg-page)", borderBottom: "0.5px solid var(--border)",
                      ...
                      {isMobile ? (
                        ...
                        <div>
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                            ...
                            <div className="tabular" style={{ fontSize: 13.5, fontWeight: 600, color: "var(--text-1)", flexShrink: 0 }}>
                              {Number(lesson.price).toLocaleString()} MDL
                            </div>
                          </div>
                          {/* Row 2: status pill + actions */}
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                            <button
                              onClick={() => handleToggle(lesson)}
                              className={`tt-pill ${lesson.isPaid ? "tt-pill-paid" : "tt-pill-unpaid"}`}
                              style={{ cursor: "pointer", border: "none", height: 26 }}
                            >
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              <button
                                ...
                                style={{ width: 32, height: 32, borderRadius: 8, color: "var(--text-3)", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg-page)", border: "0.5px solid var(--border)", cursor: "pointer" }}
                              >
                                <IcEdit />
                              </button>
                              <button
                                onClick={() => handleDelete(lesson.id)}
                                style={{ width: 32, height: 32, borderRadius: 8, color: "var(--danger)", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--danger-soft)", border: "0.5px solid color-mix(in srgb, var(--danger) 20%, transparent)", cursor: "pointer" }}
                              >
                                <IcTrash />
                              </button>
                            </div>
                        ...
                        /* ── Desktop layout: grid ── */
                        <div style={{ display: "grid", gridTemplateColumns: "56px 1fr auto auto auto auto", alignItems: "center", gap: 14 }}>
                          ...
                          <button
                            onClick={() => handleEdit(lesson)}
                            style={{ width: 28, height: 28, borderRadius: 7, color: "var(--text-3)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 120ms", background: "transparent", border: "none", cursor: "pointer" }}
