...
export default function LessonsPage() {
  ...
  const handleToggle = (l: any) =>
    togglePayment.mutate({ id: l.id, isPaid: !l.isPaid });

  const getStudent = (id: string) =>
    (students as any[]).find((s) => s.id === id);

  const fmtShort = (iso: string) => {
    const d = new Date(iso);
    return {
      day: d.getDate(),
      mon: d.toLocaleDateString("ro-RO", { month: "short" }).replace(".", ""),
    };
  };
  ...
  const fmtDow = (iso: string) =>
    new Date(iso).toLocaleDateString("ro-RO", { weekday: "long" });

  const grouped = (lessons as any[]).reduce(
    (acc, l) => {
  ...
  const totalRevenue = (lessons as any[]).reduce(
    (s, l) => s + Number(l.price),
    0,
  );
              ...
              >
                ...
                >
                  ...
                  <span className="tabular">
                    {dayLessons.length}{" "}
                    {dayLessons.length === 1 ? "lecție" : "lecții"} ·{" "}
                    {dayLessons
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          >
                            {lesson.subjectSnapshot ?? student?.subject} ·{" "}
                      ...
                      >
                        <span
                          className={`tt-dot ${lesson.isPaid ? "tt-dot-paid" : "tt-dot-unpaid"}`}
                        />
